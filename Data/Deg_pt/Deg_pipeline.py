...
from Data.Deg_pt.utils import filter2D,USMSharp
from Data.Deg_pt.utils_blur import circular_lowpass_kernel, random_mixed_kernels
from Data.Deg_pt.utils_resize import random_resizing
from Data.Deg_pt.utils_noise import random_add_gaussian_noise_pt,random_add_poisson_noise_pt
from Data.Deg_pt.utils_jpeg import DiffJPEG
from Data.Deg_pt.matlab_functions import imresize
...
class Degradation(nn.Module):
    ...
    @torch.no_grad()
    def forward_generate_kernel(self):
        # generate kernel
    ...
    @torch.no_grad()
    def forward_train_deg(self, kernel1, kernel2, sinc_kernel, gt, use_gt_usm=True):
        # gt: BCHW, Tensor
        ...
        if use_gt_usm:
            ...
            gt_usm_copy = gt.clone()

        # first degradation
        lq1 = self.jpeg_1(self.noise_1(self.resize_1(self.blur_1(gt_usm_copy, kernel1))))

        # second degradation
        lq2 = self.jpeg_2(self.noise_2(self.resize_2(self.blur_2(lq1, kernel2), ori_h, ori_w)), ori_h, ori_w, sinc_kernel)

        return gt_usm, lq2

    @torch.no_grad()
    def forward_1(self, gt_path, uint8=False):
        # read hwc 0-1 numpy
...
if __name__ == '__main__':
    ...
    deg_pipeline = Degradation(scale=2, gt_size=1752)

    gt_path = r'E:\Dataset\SR\DF2K\DF2K_HR_train\0149.png'
    gt, gt_usm, lq, kernel1, kernel2, sinc_kernel = deg_pipeline(gt_path, uint8=True)

    cv2.imwrite('lq.png', lq[:,:,::-1])
    cv2.imwrite('gt.png', gt[:,:,::-1])
    cv2.imwrite('gt_usm.png', gt[:,:,::-1])
