...
from Data.data_utils import _get_paths_from_images, paired_random_crop_, random_augmentation, _random_crop_patch
from Data.Deg_pt.Deg_pipeline import Degradation


def Dataloader(scale, gt_size, train_batchsize, val_batchsize, num_worker, mode='train_small', pin_memory=False):
    train_dataset = Dataset_PairedImage(scale,mode=mode,gt_size=gt_size)
...
class Dataset_PairedImage(Dataset):
    ...
    def __init__(self,scale=2,mode='train_small', gt_size=128, geometric_augs=True, repeat=1):
        ...
        self.scale = scale
        self.mode = mode
        assert self.mode in ('train_big','train_small')
        ...
        self.deg_kernel = Degradation(scale, gt_size)
    ...
    def __getitem__(self, index):
        ...
        img_gt = _random_crop_patch(img_gt, patch_size=self.gt_size,flag='np')

        if self.geometric_augs:
            img_gt = random_augmentation(img_gt)

        kernel1, kernel2, sinc_kernel = self.deg_kernel.forward_generate_kernel()

        img_gt = self.np2tensor(img_gt)
    ...
    #
    #     img_lq, img_gt = mosaic_dataset(self.hr, self.lr_p,self.gt_size,self.scale)
    #
...
if __name__ == '__main__':
    ...
    for i, (im,gt) in enumerate(train_dataloader):
