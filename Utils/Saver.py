...
class MySaver():
    ...
    def save_record_train_gan(self, epoch, l_pix, l_percep, l_g_gan, l_dreal, l_dfake, out_dreal, out_dfake):
        with open(os.path.join(self.path_record, 'train.txt'), 'a+') as f:
            f.write(f'{get_time()} epoch:{epoch} l_pix:{l_pix}, l_percep:{l_percep}, l_g_gan:{l_g_gan}, '
                    f'l_d_real:{l_dreal}, l_d_fake:{l_dfake}, out_d_real:{out_dreal}, out_d_fake:{out_dfake} \n')
    ...
    def save_checkpoint_noval(self, state, epoch, iter, args, print_=False, epoch_save=False, iter_save=False, save_freq=200):
        # filename = f'{epoch}_{iter}_checkpoint_{args.model}_{args.patch_size[0]}.pth'
        filepath = self.path_weights

        if epoch_save:
            filename = f'{epoch}_checkpoint_{args.model}_{args.patch_size[0]}.pth'
            torch.save(state, os.path.join(filepath, filename))
        if iter_save:
            filename = f'{epoch}_{iter}_checkpoint_{args.model}_{args.patch_size[0]}.pth'
            if iter % save_freq == 0:
    ...
    def save_configs(self,args, para, flops, h, w):
        ...
        with open(os.path.join(self.path_record, 'configs.txt'), 'a+') as f:
            ...
            f.write(f'Model Parameters:{para:,}, Flops:{flops:,} of input size:{h}x{w}')
